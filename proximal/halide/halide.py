...
def output_names(function_name, generator_source, builddir):
    ...
    # Output library
    lib_name = os.path.join(builddir, function_name + '.dll')
...
def gengen(generator_source, builddir='./build',
    ...
    generator = os.path.join(builddir, 'gengen.exe')

    # File definitions
    halide_lib = '"%HALIDE_PATH%/Release/Halide.lib"'
    ...
    try:
        ...
        # Compile
        cmd = 'cl {0} -D_HAS_EXCEPTIONS=0 -D_USE_MATH_DEFINES /GR- /MD /Z7 /O2 {1} {2} {3} {4} /Fe"{5}" /link /OPT:REF'.format(
        ...
        if verbose:
            ...
            print('\t' + cmd)
        subprocess.call(cmd, shell=True, cwd=os.path.expandvars("%HALIDE_PATH%\\Release"))

        # Find params in output generated by generator
        header_file = os.path.join(builddir, function_name + '.h')
        object_file = os.path.join(builddir, function_name + '.obj')
        ...
        cmd = ('cl.exe -D_HAS_EXCEPTIONS=0 -D_USE_MATH_DEFINES /GR- /LD /MD /Z7 /O2 {0} {1} {2} '
               '{3} /Fe"{4}" /link /EXPORT:"{5}" /OPT:REF').format(launcher_file, external_source_str,
                                            external_libs_str, object_file, output_lib, function_name_c)
        if verbose:
